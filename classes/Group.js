...
export default class Group {
  ...
  setup(teams, config) {
    (this.teams = teams), (this.config = config);
  }
  ...
  updateTeams(result) {
    ...
    if(localTeam && visitTeam) {
      ...
      //Update points
      if(result.localResult > result.visitResult) { //win localTeam
      ...
      } else if(result.visitResult > result.localResult){ //win visitTeam
  ...
  start() {
    this.matchScheduleDay.forEach(match => {
      ...
      console.log('RESULTADOS',matchSummary.results);
    })
  }
}
